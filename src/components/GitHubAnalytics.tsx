    ...
    <section className="overflow-hidden" aria-labelledby="github-analytics-title">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex gap-3 sm:gap-4">
          {user ? (
            <img src={user.avatar_url} alt={`${user.login} avatar`} className="h-10 w-10 sm:h-14 sm:w-14 rounded-full border border-line object-cover" loading="lazy" />
          ) : (
            <div className="h-10 w-10 sm:h-14 sm:w-14 rounded-full border border-line bg-paper animate-pulse" />
          )}
          <div className="min-w-0">
            <h3 id="github-analytics-title" className="text-base sm:text-lg font-semibold tracking-tight text-ink flex flex-wrap items-center gap-2">
              ...
              GitHub
              <span className="font-mono text-xs font-normal text-muted truncate">@{username}</span>
            </h3>
            {user && <p className="mt-1 text-xs sm:text-sm leading-5 sm:leading-6 text-muted line-clamp-2">{user.bio ?? "GitHub activity and repositories."}</p>}
            {loading && !user && <p className="mt-1 font-mono text-xs text-muted">Loading…</p>}
          </div>
        </div>
        <a href={`https://github.com/${username}`} target="_blank" rel="noreferrer" className="button-secondary w-full sm:w-auto !px-3 !py-2.5 sm:!py-2 text-xs shrink-0 text-center justify-center">Open GitHub →</a>
      </div>

      {/* Stats row */}
      <div className="mt-4 grid grid-cols-3 gap-2 sm:gap-3">
        <div className="rounded-md border border-line bg-paper px-2 py-2.5 sm:px-3 sm:py-3 text-center">
          <p className="font-mono text-[10px] sm:text-xs uppercase tracking-wide text-muted">Repos</p>
          <p className="mt-1 text-xl sm:text-2xl font-semibold text-ink">{user?.public_repos ?? "—"}</p>
        </div>
        <div className="rounded-md border border-line bg-paper px-2 py-2.5 sm:px-3 sm:py-3 text-center">
          <p className="font-mono text-[10px] sm:text-xs uppercase tracking-wide text-muted">Followers</p>
          <p className="mt-1 text-xl sm:text-2xl font-semibold text-ink">{user?.followers ?? "—"}</p>
        </div>
        <div className="rounded-md border border-line bg-paper px-2 py-2.5 sm:px-3 sm:py-3 text-center">
          <p className="font-mono text-[10px] sm:text-xs uppercase tracking-wide text-muted">Following</p>
          <p className="mt-1 text-xl sm:text-2xl font-semibold text-ink">{user?.following ?? "—"}</p>
        </div>
      </div>
      ...
      {analyticsConfig.github.showContributionChart && (
        <div className="mt-4">
          ...
          <div className="mt-2 overflow-hidden sm:overflow-x-auto rounded-md border border-line bg-paper p-1.5 sm:p-2">
            <div className="overflow-x-auto sm:overflow-visible [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              <img
                src={`https://ghchart.rshah.org/${username}`}
                alt={`GitHub contribution chart for ${username}`}
                loading="lazy"
                className="w-full max-w-full sm:min-w-[640px] object-contain"
                onError={(e) => ((e.target as HTMLImageElement).style.display = "none")}
              />
            </div>
          </div>
          <p className="hidden sm:block mt-1.5 font-mono text-[11px] text-muted">Source: ghchart.rshah.org · updates daily</p>
        </div>
      )}
      ...
      <div className="mt-4">
        ...
        {loading ? (
          <div className="mt-2 grid gap-2 sm:gap-3 grid-cols-1 sm:grid-cols-2">
            {[0, 1, 2, 3].map((i) => (
              <div key={i} className="h-20 sm:h-24 rounded-md border border-line bg-paper animate-pulse" />
            ))}
          </div>
        ) : pinned.length === 0 ? (
          ...
          <div className="mt-2 grid gap-2 sm:gap-3 grid-cols-1 sm:grid-cols-2">
            {pinned.map((r) => (
              <a key={r.name} href={r.html_url} target="_blank" rel="noreferrer" className="group rounded-md border border-line bg-paper p-2.5 sm:p-3 transition hover:border-ink hover:shadow-sm">
                <p className="flex items-center justify-between gap-2 text-sm font-semibold text-ink group-hover:text-rust">
                  <span className="truncate text-[13px] sm:text-sm">{r.name}</span>
